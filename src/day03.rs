...
fn strings_to_nums(lines: &Vec<String>) -> Vec<u32> {
    lines
        .iter()
...
fn most_common_bits(nums: &Vec<u32>, num_bits: usize) -> Vec<u32> {
    let mut bit_sums = vec![0; num_bits];

    nums.iter().fold(&mut bit_sums, |acc, x| {
        for i in 0..num_bits {
            acc[i] += (x >> (num_bits - 1 - i)) & 1;
        }
        acc
    });

    for sum in &mut bit_sums {
        *sum = if *sum as usize >= (nums.len() + 1) / 2 { // Ceiling rounding
            1
        } else {
            0
        };
...
fn part1(lines: &Vec<String>) -> u32 {
    ...
    let gamma = bit_vec_to_num(most_common_bits(&strings_to_nums(lines), num_bits));
...
fn part2(lines: &Vec<String>) -> u32 {
    let num_bits = lines[0].len();
    let oxygen_generator_rating = iterative_filter(&mut strings_to_nums(lines), num_bits, true);
    let co2_scrubber_rating = iterative_filter(&mut strings_to_nums(lines), num_bits, false);
